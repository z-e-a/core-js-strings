...
function formatTime(minutes, seconds) {
  // return minutes.toPrecision(2) + seconds.toPrecision(2);
  // return minutes.toPrecision(2) + seconds.toPrecision(2);
...
function isPalindrome(str) {
  ...
  let result = true;
  for (let i = 0; i < Math.floor(sourceStr.length / 2); i += 1) {
    result = result && sourceStr[i] === sourceStr[sourceStr.length - i - 1];
  }
  return result;
}
...
function encodeToRot13(str) {
  ...
  return str
    ...
    .map((c) => {
      let result = c;
      if (capitals.includes(c)) {
        result = capitals[(capitals.indexOf(c) + 13) % capitals.length];
      }
      if (lowercase.includes(c)) {
        result = lowercase[(lowercase.indexOf(c) + 13) % lowercase.length];
      }
      return result;
    })
    .join('');
